...
package com.salesforce.mirus.utils;
...
public class MockTime implements Time {
  ...
  // Values from `nanoTime` and `currentTimeMillis` are not comparable, so we store them separately
  // to allow tests
  ...
  public void waitObject(Object obj, Supplier<Boolean> condition, long deadlineMs)
    ...
    try {
      synchronized (obj) {
        ...
        if (!condition.get()) throw new TimeoutException("Condition not satisfied before deadline");
      }
    } finally {
